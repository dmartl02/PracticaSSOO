...
struct atleta {
	...
	int id;  //numero de atletas (de 1 a 10)
	int ha_Competido;
	int tarima_Asignada;
	int necesita_Beber;
};
...
int main() {
	...
	idTarima1 = 1;
	idTarima2 = 2;
	contadorAtletas = 0;  //posicion de los atletas (de 0 a 9)
...
void *AccionesAtleta(void *arg) {
	char id[10];
	char msg[100];
	int idAtleta = *(int *)arg;  //numero del atleta (de 1 a 10)
	int posicion, i;
	...
	/*do {  //Espera activa en la cual se mide el pulso
			...
			if (num <= 15) {
				...
				writeLogMessage(id, msg);
				pthread_mutex_unlock(&escritura);
				printf("FIN HILO %d ELECTROLITOS\n", idAtleta);
				//pthread_exit(NULL);
				pthread_cancel(punteroAtletas[idAtleta-1].atletaHilo);  //HE HECHO UN CAMBIO -1
			}
			sleep(3);
	} while(punteroAtletas[idAtleta-1].ha_Competido == 0);*/
...
void *AccionesTarima(void *arg) {
        ...
        if (colaTarimas[0] != 50) {  //Forma de escoger los atletas 
            ...
            if (atletaActual != 50) {
	            ...
	            if (eventoAleatorio <= 80) {  //Movimiento valido
					...
					comprobarPodio(puntuacionAleatorio, atletaActual);
					atletasAtendidos++;
					pthread_cancel(punteroAtletas[atletaActual].atletaHilo);
	...
	/*while(tarimaOcupada == 0 && punteroAtletas[atletaActual].ha_Competido == 0) {
		printf("COMIENZO WHILE soy %d\n", idTarima);
		if (colaTarimas[0] != 50) {  //Forma de escoger los atletas 
	        pthread_mutex_lock(&entradaTarima);
	        atletaActual = colaTarimas[0];  //Identificamos el atleta
	        for(i = 1; i <= ATLETAS; i++){
	...
	//EL PROBLEMA ES QUE EL ATLETA SUBE A COMPETIR A LA TARIMA Y EN PLENA ACCION SE RETIRA POR DESHIDRATACION
	//Y FALTA DE ELECTROLITOS CUANDO AL SUBIR A LA TARIMA CAMBIAMOS EL FLAG DE HA_COMPETIDO
	//ENTONCES NO DEBERIA MIRARSE EL PULSO YA QUE YA HA SUBIDO A LA TARIMA

	//OTRO PROBLEMAS ES QUE UN MISMO ATLETA COMPITE VARIAS VECES Y SUBE A LAS DOS TARIMAS ENTONCES APARECEN VARIAS
	//PUNTUACIONES DISTINTAS

	//LLEVO MUCHO TIEMPO MIRANDO LOS FALLOS Y NO SOY CAPAZ A RESOLVERLOS, HAY VARIAS VERSIONES DEL ACCIONESTARIMA
	//POR SI QUIERES ECHARLAS UN VISTAZO

	//VERSION 3

	while(tarimaOcupada == 0) {
		for (i = 0; i < ATLETAS; i++) {
			if (colaTarimas[i] != 50 && punteroAtletas[i].tarima_Asignada == idTarima && punteroAtletas[i].ha_Competido == 0) {  //Si la tarima asignada al atleta coincide con la tarima actual 
				printf("Buenas gente soy %d\n", punteroAtletas[i].id);
			...
			//Si la tarima asignada al atleta no coincide con la tarima actual (una tarima ayuda a la otra)
			if (colaTarimas[i] != 50 && punteroAtletas[i].ha_Competido == 0) {
				printf("Buenas gente soy %d\n", punteroAtletas[i].id);
	...
	printf("FIN WHILE, mi id = %d, el atleta fue %d\n", idTarima, atletaActual);
}
