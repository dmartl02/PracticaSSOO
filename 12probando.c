		...
		for (i = 0; i < ATLETAS; i++) {
			if (colaTarimas[i] != 50 && punteroAtletas[i].tarima_Asignada == idTarima && punteroAtletas[i].ha_Competido == 0) {  //Si la tarima asignada al atleta coincide con la tarima actual 
		        ...
		        if (eventoAleatorio <= 80) {  //Movimiento valido
					...
					///////////////////////////////////////////////////
					aguaAleatorio = generarAleatorio(1, 100);
...
void *necesitaBeber(void *arg){
	...
	int competidoresBebiendo;
	int i;
	while(1) {
		for (i = 0; i < ATLETAS; i++) {
			if (punteroAtletas[i].necesita_Beber = 1){
				printf("El atleta %d necesita beber",punteroAtletas[i].id);
			}
		}
	sleep(2);
	}
	//printf("Han ido a beber %d atletas", competidoresBebiendo);
}
